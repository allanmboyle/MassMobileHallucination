...
console.log("unique: incoming update id: " + update.id);
console.log("unique: existing id data: " + uniqueUpdates[update.id]);

    if (uniqueUpdates[update.id]) {
console.log("unique: existing!")
...
console.log("unique: new!")
    ...
    console.log("There are curent " + Object.keys(uniqueUpdates).length + " unique player updates.")
    // store the current value as the old for next time
console.log("unique: storing update.accel: " + update.accel);

    uniqueUpdates[update.id] = update.accel;

console.log("unique: just stored the new one so it should be there: " + uniqueUpdates[update.id]);
}
...
function periodicPlayfieldUpdate() {
    ...
    if (updates.length > 0) {
        ...
        if (!no_totals) {
            ...
            // these get processed the same, but you can't request both.
